...
class WrestlingScoringApp:
    ...
    def show_intro_screen(self):
        ...
        tk.Label(self.left_frame, text="Wrestler 1 Name", font=("Arial", 24, "bold"), bg="red", fg="white").pack(pady=100)
        self.name1_entry = tk.Entry(self.left_frame, font=("Arial", 24), justify="center")
        self.name1_entry.pack(pady=20)

        tk.Label(self.right_frame, text="Wrestler 2 Name", font=("Arial", 24, "bold"), bg="blue", fg="white").pack(pady=100)
        self.name2_entry = tk.Entry(self.right_frame, font=("Arial", 24), justify="center")
        self.name2_entry.pack(pady=20)
    ...
    def handle_decision_result(self):
        ...
        if self.score1 > self.score2:
            ...
            max1 = max([p for p, s, _ in self.history if s == 1], default=0)
            max2 = max([p for p, s, _ in self.history if s == 2], default=0)
            if max1 > max2:
                winner = 1
            elif max2 > max1:
                winner = 2
    ...
    def end_match(self, method, winner):
        ...
        tk.Label(self.root, text=f"{name1 if winner == 1 else name2} WINS by {method}",
